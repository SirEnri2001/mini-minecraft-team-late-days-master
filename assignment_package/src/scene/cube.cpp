...
void Cube::createVBOdata()
{
    ...
    GLCall(mp_context->glBufferData(GL_ELEMENT_ARRAY_BUFFER, CUB_IDX_COUNT * sizeof(GLuint), sph_idx, GL_STATIC_DRAW));

    // The next few sets of function calls are basically the same as above, except bufPos and bufNor are
    // array buffers rather than element array buffers, as they store vertex attributes like position.
    generatePos();
    GLCall(mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufPos));
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, CUB_VERT_COUNT * sizeof(glm::vec4), sph_vert_pos, GL_STATIC_DRAW));

    generateNor();
    GLCall(mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufNor));
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, CUB_VERT_COUNT * sizeof(glm::vec4), sph_vert_nor, GL_STATIC_DRAW));
...
void Cube::createInstancedVBOdata(std::vector<glm::vec3> &offsets, std::vector<glm::vec3> &colors) {
    ...
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, offsets.size() * sizeof(glm::vec3), offsets.data(), GL_STATIC_DRAW));


    generateCol();
    GLCall(mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufCol));
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, colors.size() * sizeof(glm::vec3), colors.data(), GL_STATIC_DRAW));
}
