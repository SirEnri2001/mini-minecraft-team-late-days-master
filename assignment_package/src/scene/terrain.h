...
const static int TERRAIN_SIZE = 64;
const static float TERRAIN_GENERATE_CD = 2;
const static int GENERATION_ZONE_SIZE = 2;
...
private:
    ...
    float m_terrainGenTimer{0};

    MultithreadTerrainGenerator m_terrainGenerator;
...
public:
    ...
    bool hasChunkAt(float x, float z) const;
    // Do these world-space coordinates lie within
    // a Terrain that exists?
    ...
    void updateneighborchunk(int blocklocalX,int blocklocalz,int chunkoriginx,int chunkoriginz,std::unordered_set<Chunk *> &updatechunks);
    // function used to simulate fluid
    void fluidsimulation(int x, int y, int z, BlockType t, std::unordered_set<Chunk *> &updatechunks);
    // Given a world-space coordinate (which may have negative
    ...
    void drawOpaque(ShaderProgram *shaderProgram);

    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.
    void CreateTestScene();

    // if a player's position is near a chunk's edge within a distance of "proximity"
    // and that edge has no neighbor, add a new chunk.
    void tryExpand(float playerX, float playerZ, float dT);

    // helper functions for world-local coordinate transformation
    static int clampToCoordinate(float x, int len);
};
