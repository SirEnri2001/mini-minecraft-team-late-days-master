...
void Quad::createVBOdata()
{
    ...
    glm::vec4 vert_col[4] {glm::vec4(0.5f, 0.5f, 0.6f, 1.f),
    ...
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec4), vert_pos, GL_STATIC_DRAW));
    GLCall(mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufUV));
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(glm::vec2), vert_UV, GL_STATIC_DRAW));
    generateCol();
    GLCall(mp_context->glBindBuffer(GL_ARRAY_BUFFER,m_bufCol));
    GLCall(mp_context->glBufferData(GL_ARRAY_BUFFER,4 * sizeof(glm::vec4), vert_col, GL_STATIC_DRAW));
}
